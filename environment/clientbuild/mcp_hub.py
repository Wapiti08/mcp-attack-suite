...
from environment.clientbuild.logging_setup import safe_json_dumps
...
def _unwrap_call_tool_result(result: Any) -> Any:
    """
    Different MCP client implementations may return:
    - plain Python values (dict/list/str)
    - MCP `CallToolResult` objects with `.content` (list of content items)

    The rest of this codebase expects a plain Python value, so we unwrap best-effort.
    """
    ...
    if isinstance(content, list):
        ...
        for item in content:
            ...
            if item_type in {"json", "application/json"}:
                payload = _get_attr_or_key(item, "json") or _get_attr_or_key(item, "data")
    ...
    # Pydantic models / dataclasses / other objects: try common conversions.
...
class MCPHub:
    ...
    def __init__(self, servers: list[tuple[str, Any]], *, trace_callback: Callable[[dict[str, Any]], None] | None = None):
    ...
    def _trace(self, event: dict[str, Any]) -> None:
        if self._trace_callback is None:
            return
        try:
            self._trace_callback(dict(event))
        except Exception:
            return
    ...
    def from_urls(
        ...
        *,
        ...
        return cls(servers, trace_callback=trace_callback)

    async def __aenter__(self) -> "MCPHub":
        self._stack = AsyncExitStack()
        self._logger.info("mcp_hub.connect servers=%s", [name for name, _ in self._servers])
        self._trace({"event": "mcp.connect", "servers": [name for name, _ in self._servers]})
    ...
    async def refresh_tools(self) -> None:
        ...
        self._trace(
            {
                "event": "mcp.tools",
                "count": len(self._tool_specs),
                "tools": [t.name for t in self._tool_specs],
            }
        )
    ...
    async def call_tool(self, exposed_name: str, arguments: Mapping[str, Any]) -> Any:
        ...
        for server_name, client in self._servers:
            if server_name == ref.server_name:
                ...
                result = _unwrap_call_tool_result(result)
                self._trace(
                    {
                        "event": "mcp.result",
                        "exposed": exposed_name,
