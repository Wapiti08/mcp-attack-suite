...
def load_settings_with_mcp(mcp_urls: list[str], mcp_names: list[str] | None) -> Settings:
    try:
        ...
        raise RuntimeError("Missing dependency: python-dotenv. Install it with `pip install python-dotenv`.") from e

    root = Path(__file__).resolve().parents[1]
    load_dotenv(dotenv_path=root / ".env", override=False)
    load_dotenv(dotenv_path=root / "clientbuild" / ".env", override=True)
    ...
    return Settings(
        ...
        log_dir=str(LOGGING.get("dir", "clientbuild/logs")),
